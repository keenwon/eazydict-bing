...
function _parseTrans($containor) {
  ...
  let count = configs.output.translates === -1
    ? Infinity
    : configs.output.translates;

  $containor.find('li').slice(0, count).each((index, item) => {
    let $li = $(item);
...
function _parseExamples($containor) {
  ...
  let count = configs.output.examples === -1
    ? Infinity
    : configs.output.examples;

  $containor.find('#sentenceSeg .se_li1').slice(0, count).each((index, item) => {
